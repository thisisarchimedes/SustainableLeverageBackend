...
describe('Rebalance pool', function() {
  ...
  it('Rebalance pegged curve pool', async function() {
    ...
    await evmStorage.setERC20Balance(FRAXBP, fraxbpMemSlot.slot,
        new EthereumAddress(signer.address), curvePool.valueTokenBalance * 100n, fraxbpMemSlot.isVyper);

    // Assert the pool is unbalanced
    const alUSDPriceInFRAXBPBefore = await curvePool.getDumpTokenPriceInValueToken();
    assert(Number(ethers.formatUnits(alUSDPriceInFRAXBPBefore,
        curvePool.valueTokenDecimals)) < 0.75, 'Pool is not unbalanced at start');
    ...
    assert.approximately(Number(ethers.formatUnits(alUSDPriceInFRAXBPAfter, curvePool.valueTokenDecimals)),
        1, 0.01, 'Pool did not balance');
  });
});
