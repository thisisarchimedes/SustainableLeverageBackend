...
import {ALUSD, FRAXBP, getTokenBalancesSlot} from '../test/lib/addresses';
...
(async () => {
  ...
  let curvePool = await CurvePool.createInstance(signer, ALUSD, FRAXBP);
  ...
  curvePool = await CurvePool.createInstance(signer, ALUSD, FRAXBP);
  console.log('FRAXBP', curvePool.valueTokenBalance.toString());
  console.log('ALUSD', curvePool.dumpTokenBalance.toString());
  console.log('1 ALUSD = ', await (curvePool.getDumpTokenPriceInValueToken()), 'FRAXBP');
})();
